        ...
        if (
            ...
            if supported_currencies:
                if announcement_coin in supported_currencies:
                    ...
                    obj = get_last_price(
                        announcement_coin, globals.pairing, False
                    )
                    price = obj.price
                    ...
                    try:
                        ...
                        if globals.test_mode:
                            if (
                                order[announcement_coin]["_status"]
                                == "cancelled"
                            ):
                                status = "closed"
                                left = 0
                            ...
                            order[announcement_coin] = {
                                ...
                                "_id": "test-order",
                                "_text": "test-order",
                                "_create_time": datetime.timestamp(
                                    datetime.now()
                                ),
                        ...
                        # place a live order if False
                            ...
                            order[announcement_coin] = order[
                                announcement_coin
                            ].__dict__
                            order[announcement_coin].pop(
                        ...
                        if order_status == "closed":
                            ...
                            session[announcement_coin]["total_volume"] += float(
                                order[announcement_coin]["_amount"]
                            ) * float(order[announcement_coin]["_price"])
                            session[announcement_coin]["total_amount"] += float(
                                order[announcement_coin]["_amount"]
                            )
                            session[announcement_coin]["total_fees"] += float(
                                order[announcement_coin]["_fee"]
                            )
                            ...
                            if (
                                ...
                                logger.info(
                                    f"Partial fill order detected.  {order_status=} | {partial_amount=} out of {amount=} | {partial_fee=} | {price=}"
                                )
                            ...
                            logger.info(
                                f"Clearing order with a status of {order_status}.  Waiting for 'closed' status"
                            )
                            order.pop(
                                announcement_coin
    ...
    while not globals.stop_threads:
        ...
        if len(order) > 0:
            for coin in list(order):
                ...
                top_position_price = stored_price + (
                    stored_price * coin_tp / 100
                )
                ...
                if (
                    ...
                    and globals.enable_tsl
                ):
                    # increase as absolute value for TP
                    new_tp = float(last_price) + (
                        float(last_price) * globals.ttp / 100
                    )
                    ...
                    new_sl = float(last_price) + (
                        float(last_price) * globals.tsl / 100
                    )
                    ...
                    new_top_position_price = stored_price + (
                        stored_price * new_tp / 100
                    )
                    new_stop_loss_price = stored_price + (
                        stored_price * new_sl / 100
                    )
                ...
                # close trade if tsl is reached or trail option is not enabled
                    ...
                    float(last_price)
                    < stored_price + (stored_price * coin_sl / 100)
                    or float(last_price)
                    > stored_price + (stored_price * coin_tp / 100)
                    ...
                    try:
                        ...
                        if not globals.test_mode:
                            ...
                            if sell._status != "closed":
                                ...
                                if float(sell._left) > 0 and float(
                                    sell._amount
                                ) > float(sell._left):
                    ...
                    # store sold trades data
                        ...
                        if not globals.test_mode:
                            ...
                            sold_coins[coin][
                                "profit"
                            ] = f"{float(last_price) - stored_price}"
                            sold_coins[coin][
                                "relative_profit_%"
